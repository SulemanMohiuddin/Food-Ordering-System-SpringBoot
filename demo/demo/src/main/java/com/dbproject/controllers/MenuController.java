...
public class MenuController {
    ...
    @Autowired
    public MenuController(MenuService menuService, OrderService orderService, LoginService loginService) {
    ...
    public String addToCart(@RequestParam String itemName,
                            ...
                            @RequestParam String itemSeller,
        ...
        try {
            ...
            cartItem.setItemSeller(itemSeller);
            cartItem.setItemtype(itemType);
            Integer restaurantId = restaurantService.getRestaurantIdByName(itemSeller);
...
//        }
    ...
    public ResponseEntity<String> sendItem() {
            ...
            orderModel.setOitems(cartItemsToString(cartItems)); // Convert items to a single string

        IntStream id = cartItems.stream().mapToInt(CartItem::getRestaurantId);



           orderService.createOrder(totalPrice,"pending", id.sum(), 6,orderModel.getOitems());


            cartItems.clear();

            return ResponseEntity.ok("Item sent successfully");

        }



    private String cartItemsToString(List<CartItem> cartItems) {
        return cartItems.stream().map(CartItem::getItemName).reduce((item1, item2) -> item1 + ", " + item2).orElse("");
    }
